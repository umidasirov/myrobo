...
//       setCatLoading(true);
//       try {
//         const res = await fetch("https://myrobo.adxamov.uz/api/blog/categories/");
...
//     setLoad(true);
//     try {
//       const res = await fetch("https://myrobo.adxamov.uz/api/blog/blog-create/", {
//         method: "POST",
//         body: formData,
...
export default function ArticleEditor({ onClose }) {
  ...
  // Type state (articles uses "type" instead of "category")
  ...
  useEffect(() => {
    const fetchTypes = async () => {
      setTypeLoading(true);
      try {
        const res = await fetch("https://myrobo.adxamov.uz/articles/types/");
        if (!res.ok) throw new Error();
        const data = await res.json();
        setTypes(Array.isArray(data) ? data : (data.results ?? []));
      } catch {
        notification.error({ message: "Turlarni yuklashda xatolik" });
      } finally {
  ...
  const handleSubmit = async () => {
    if (!title.trim()) return notification.error({ message: "Sarlavha majburiy" });
    if (!selectedType) return notification.error({ message: "Tur majburiy" });
    if (!editorData.trim()) return notification.error({ message: "Kontent majburiy" });
    if (!banner?.file) return notification.error({ message: "Banner rasm majburiy" });

    const formData = new FormData();
    formData.append("title", title);
    formData.append("type_slug", selectedType);       // "type" field (not "category")
    formData.append("description", editorData);
    formData.append("image", banner.file);       // "image" field (not "img")
    const navigate = useNavigate()
    setLoad(true);
    try {
      const res = await fetch("https://myrobo.adxamov.uz/articles/create/", {
        method: "POST",
        body: formData,
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <>
      <Helmet>
        <title>Maqola qo'shish - MyRobo</title>
        <meta name="description" content="MyRobo ga yangi maqola qo'shing." />
        <meta name="keywords" content="maqola qo'shish, articles, yozish, MyRobo" />
        <meta property="og:title" content="Maqola qo'shish - MyRobo" />
        <meta property="og:description" content="MyRobo ga yangi maqola qo'shing." />
        <meta property="og:type" content="website" />
      </Helmet>
      <div className="flex items-center justify-center p-4">
        <div className="relative rounded-3xl flex flex-col w-full max-w-3xl gap-2">
            ...
            <div>
              ...
              <select
                ...
                style={{
                  background: "rgba(255,255,255,0.04)",
                  border: "1px solid rgba(160,180,255,0.28)",
                  color: selectedType ? "#111" : "#5a6480",
                }}
                onFocus={(e) => (e.target.style.borderColor = "rgba(100,140,255,0.6)")}
                onBlur={(e) => (e.target.style.borderColor = "rgba(160,180,255,0.28)")}
              >
                <option value="" disabled>
                  {typeLoading ? "Yuklanmoqda…" : "Turni tanlang"}
                </option>
                {types.map((type) => (
