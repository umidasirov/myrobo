...
//       setCatsLoading(true);
//       try {
//         const res = await apiFetch("https://myrobo.adxamov.uz/api/blog/categories/");
...
//       try {
//         const res = await apiFetch("https://myrobo.adxamov.uz/api/blog/blogs/");
...
//         [...slugSet].map((slug) =>
//           fetch(`https://myrobo.adxamov.uz/api/blog/blogs/?category=${slug}`, {
...
//         if (onClick) onClick();
//       }}
//       className="group bg-white dark:bg-gray-800 rounded-xl md:rounded-2xl border border-gray-100 dark:border-gray-700 cursor-pointer
...
//                  hover:border-blue-100 dark:hover:border-blue-700 transition-all duration-300 overflow-hidden hover:overflow-hidden"
//     >
//       <div className="relative h-40 md:h-48 overflow-hidden bg-blue-50 dark:bg-gray-700">
//         <img
//           src={b.img}
//           alt={b.title}
//           className="w-full h-full rounded-t-lg rounded-r-lg object-cover group-hover:scale-110 transition-transform duration-500"
//           loading="lazy"
//           onError={(e) => {
...
export default function ArticleComponents() {
  ...
  useEffect(() => {
    const fetchTypes = async () => {
      ...
      try {
        const res = await apiFetch("https://myrobo.adxamov.uz/articles/types/");
        const data = await res.json();
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      try {
        const res = await apiFetch("https://myrobo.adxamov.uz/articles/articles/");
        const data = await res.json();
  ...
  const fetchBySelected = useCallback(async (slugSet) => {
    if (slugSet.size === 0) {
      if (searchQuery.trim()) {
        const query = searchQuery.toLowerCase();
        setArticleData(allArticles.filter((a) =>
    ...
    try {
      const results = await Promise.all(
        [...slugSet].map((slug) =>
          fetch(`https://myrobo.adxamov.uz/articles/articles/?type=${slug}`, {
            headers: { "Content-Type": "application/json" },
          }).then((r) => r.json()).then((d) => Array.isArray(d) ? d : (d.results ?? []))
        )
      );
      const merged = Object.values(
        results.flat().reduce((acc, a) => ({ ...acc, [a.slug]: a }), {})
      );
      let filtered = merged;
      if (searchQuery.trim()) {
        const query = searchQuery.toLowerCase();
  ...
  const goTo = (slug) => {
    if (!slug) return;
    navigate(`/articles/${slug}`, { state: { name: slug } });
  };
    ...
    <section className="w-full md:w-[90%] max-w-[1400px] mx-auto mt-6 md:mt-10 px-4 md:px-0 pb-16 md:pb-20">
      <div className="mb-6 md:mb-8 flex flex-col gap-4 md:gap-6">
        ...
        {articlesLoading ? (
          ...
          <div className="grid gap-4 md:gap-5 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {articleData.map((a) => (
              <ArticleCard key={a.slug} article={a} onClick={() => goTo(a.slug)} />
            ))}
          </div>
        )}
      </div>
...
function ArticleCard({ article: a, onClick }) {
  ...
  return (
    <div
      ...
      className="group bg-white dark:bg-gray-800 rounded-xl md:rounded-2xl border border-gray-100 dark:border-gray-700 cursor-pointer
                 flex flex-col hover:-translate-y-1 md:hover:-translate-y-2 hover:shadow-lg md:hover:shadow-xl hover:shadow-blue-100 dark:hover:shadow-blue-900/20
                 hover:border-blue-100 dark:hover:border-blue-700 transition-all duration-300 overflow-hidden"
    >
      <div className="relative h-40 md:h-48 overflow-hidden bg-blue-50 dark:bg-gray-700">
        <img
          src={a.image}
      ...
      <div className="flex flex-col gap-2 p-3 md:p-4 flex-1">
        ...
        <p className="text-xs md:text-sm text-gray-400 dark:text-gray-500 line-clamp-2 flex-1 leading-relaxed">
          {truncate(stripHtml(a.description), 15)}
        </p>
        <div className="flex items-center gap-3 md:gap-4 pt-2 md:pt-3 mt-auto border-t border-gray-100 dark:border-gray-700">
          <MetaItem icon={<EyeOutlined />} label={a.views ?? 0} />
          <MetaItem icon={<CalendarOutlined />} label={formatDate(a.created_at)} />
        </div>
      </div>
    </div>
  );
