...
function SertificateComponents() {
  return (
    <motion.section
      className="mt-5 rounded-2xl bg-[#f1f2f7] dark:bg-gray-900 py-8 sm:py-10 lg:py-12 transition-colors duration-300"
      initial="hidden"
      whileInView="visible"
        ...
        >
          {servicesData.map((item, index) => (
            <motion.div
              ...
              className="group flex items-start gap-4 rounded-2xl bg-white dark:bg-gray-800 p-4 shadow-sm border border-transparent dark:border-gray-700 transition-all duration-300 hover:shadow-xl sm:p-5 lg:p-6"
            >
              <div className="flex h-[64px] w-[64px] shrink-0 items-center justify-center rounded-full bg-blue-50 dark:bg-blue-900/30 shadow-md shadow-blue-100 dark:shadow-blue-900/20 transition-transform duration-300 group-hover:scale-110 sm:h-[72px] sm:w-[72px]">
                {item.icon}
              </div>

              <div className="flex-1">
                <h3 className="text-[18px] font-semibold leading-tight text-gray-900 dark:text-white sm:text-[20px] lg:text-[22px]">
                  {item.title}
                </h3>

                <p className="mt-2 text-sm leading-6 text-gray-500 dark:text-gray-400 sm:text-[15px] lg:text-base">
                  {item.description}
                </p>
              </div>
            </motion.div>
